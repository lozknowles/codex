...
export CODEX_TERMUX_ANDROID_NDK_VERSION CODEX_TERMUX_BUILD_PROFILE
...
# This is derived rather than configured independently so the compiler target
# cannot drift away from the audited Rust target and Android API level.
CODEX_TERMUX_ANDROID_CLANG_TARGET="${CODEX_TERMUX_TARGET}${CODEX_TERMUX_ANDROID_API}"
export CODEX_TERMUX_ANDROID_CLANG_TARGET
