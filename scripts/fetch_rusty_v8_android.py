...
def main() -> int:
    ...
    expected_release_tag = f"rusty-v8-v{version}"
    if release_tag != expected_release_tag:
        raise SystemExit(
            "manifest release tag does not match the resolved v8 crate version; "
            f"expected {expected_release_tag}, got {release_tag}"
        )
    repository = os.environ.get("CODEX_TERMUX_V8_REPOSITORY") or manifest["repository"]
    ...
    print(f"export CODEX_TERMUX_RESOLVED_V8_VERSION={shlex.quote(version)}")
    print(
