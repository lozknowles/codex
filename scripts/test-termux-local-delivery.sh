...
if python3 "$repo_root/scripts/fetch_rusty_v8_android.py" \
  --release-tag rusty-v8-v149.2.0 --output-dir "$test_root/v8" \
  >"$test_root/v8-tag.out" 2>"$test_root/v8-tag.err"; then
  fail "mismatched V8 release tag was accepted"
fi
grep -q 'cannot replace the checksum-pinned manifest tag' "$test_root/v8-tag.err" \
  || fail "unexpected V8 release-tag validation error"
...
cp "$repo_root/scripts/rollback-local-termux.sh" "$bundle/rollback.sh"
...
chmod 0755 "$bundle/bin/codex" "$bundle/bin/codex.bin" "$bundle/install.sh" \
  "$bundle/uninstall.sh" "$bundle/rollback.sh" "$fake_path/uname"
...
cp -R "$prefix/libexec/codex-termux-local" \
  "$prefix/libexec/codex-termux-local.previous"
printf '%s\n' 'previous payload' > "$prefix/libexec/codex-termux-local.previous/marker"
PREFIX="$prefix" PATH="$fake_path:$PATH" TERMUX_VERSION=test \
  CODEX_TERMUX_TEST_LOG="$test_log" sh "$bundle/rollback.sh"
[ -f "$prefix/libexec/codex-termux-local/marker" ] \
  || fail "rollback did not activate the previous installation"
[ -x "$prefix/libexec/codex-termux-local.previous/codex.bin" ] \
  || fail "rollback did not retain the replaced installation"

PREFIX="$prefix" PATH="$fake_path:$PATH" TERMUX_VERSION=test \
  CODEX_TERMUX_TEST_LOG="$test_log" sh "$bundle/rollback.sh"
[ -x "$prefix/libexec/codex-termux-local/codex.bin" ] \
  || fail "second rollback did not restore the newer installation"
